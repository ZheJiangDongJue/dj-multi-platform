  ...
  <div class="batch-receive">
    ...
    <div class="batch-receive__scan-container">
      <div class="batch-receive__scan-section">
        <div class="batch-receive__scan-title">制令单号扫码区</div>
        <div class="batch-receive__scan-area">
          <input ref="flowScanInput" v-model="flowScanCode" class="batch-receive__scan-input" placeholder="在这扫描制令单号"
      ...
      <div v-else class="batch-receive__card-grid">
        <div v-for="(item, index) in itemList" :key="index" class="card" :class="{ 'card--selected': item.selected }"
          ...
          <div class="card__content">
            ...
            <div class="card__input-row">
              <label class="card__label"><span class="required-field">*</span>接收数量:</label>
  ...
  methods: {
    ...
    async checkAndAddFlowRecord(code) {
      try {
        ...
        query.ShortName = 'dpd';
        query.Select = 'dpd.id';
        query.AddWhere(`dpd.CodeForScan = '${code}' or dpd.InnerKey = '${code}'`); // 使用扫码值查询对应记录
