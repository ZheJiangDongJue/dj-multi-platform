<template>
  <div class="batch-receive">
    ...
    <!-- 头部扫码输入区域 - 修改为左右两部分 -->
    <div class="batch-receive__scan-container">
      <div class="batch-receive__scan-section">
        <div class="batch-receive__scan-title">流程卡扫码区</div>
        <div class="batch-receive__scan-area">
          <input ref="flowScanInput" v-model="flowScanCode" class="batch-receive__scan-input" 
            placeholder="请在这里扫描流程卡编码" @keyup.enter="handleFlowScanCodeConfirm" autofocus />
        </div>
      </div>
      <div class="batch-receive__scan-section">
        <div class="batch-receive__scan-title">员工扫码区</div>
        <div class="batch-receive__scan-area">
          <input ref="employeeScanInput" v-model="employeeScanCode" class="batch-receive__scan-input" 
        ...
        <div v-for="(item, index) in itemList" :key="index" class="card"
          ...
          <div class="card__content">
            ...
            <div class="card__input-row">
              ...
              <input type="number" v-model.number="item.qty" min="1" :max="item.data?.CanReceiveQty || 999999"
                class="card__input" />
            </div>
          </div>
          <!-- 卡片底部，可以添加额外状态或按钮 -->
    ...
    <div class="batch-receive__submit">
      <div class="batch-receive__button-wrapper">
      ...
      try {
        ...
        if (response.Status === 200 && response.Data && response.Data.length > 0) {
          ...
          if (this.itemList.length > 0) {
            ...
            if (hasSelectedItems) {
              // 有选中项目，询问是否只替换选中的项目
              this.$confirm(`是否将选中的 ${this.selectedItems.length} 个项目的接收人更改为 ${employee.Name}？`, '确认操作', {
                ...
                this.$message.success(`已将选中的 ${this.selectedItems.length} 个项目的接收人设置为: ${employee.Name}`);
              }).catch(() => {
                this.$message.info('已取消操作');
              });
            } else {
              // 没有选中项目，询问是否替换所有项目
              this.$confirm(`是否将所有 ${this.itemList.length} 个项目的接收人更改为 ${employee.Name}？`, '确认操作', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
              }).then(() => {
                ...
                this.$message.success(`已将所有 ${this.itemList.length} 个项目的接收人更改为: ${employee.Name}`);
              }).catch(() => {
                this.$message.info('已取消操作');
              });
            }
...
.batch-receive {
  ...
  &__card-grid {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(22vw, 1fr)); /* 减小最小宽度从25vw到22vw */
    gap: 1.3vh 1.04vw; /* 进一步减小间距 */
    padding: 0 1.56vw;
  }
...
.card {
  ...
  &__header {
    ...
    align-items: center;
    padding: 0.78vh 0.98vw; /* 进一步减小内边距 */
  ...
  &__content {
    padding: 1.04vh 0.98vw; /* 进一步减小内边距 */
    flex: 1;
  }
